...
double arduinoFFT::MajorPeak(double *vD, uint16_t samples, double samplingFrequency) 
{
 ...
 Serial.println(String(vD[IndexOfMaxY-1]) + " " + String(vD[IndexOfMaxY]) + " " + String(vD[IndexOfMaxY+1])+" " + String(delta));
	double interpolatedX = ((IndexOfMaxY + delta)  * samplingFrequency) / (samples-1);
	/* retuned value: interpolated frequency peak apex */
	return(interpolatedX);
}

double arduinoFFT::MajorPeakAmplitude(double *vD, uint16_t samples, double samplingFrequency) 
{
// https://www.dsprelated.com/freebooks/sasp/Sinusoidal_Peak_Interpolation.html
	...
	double alpha = vD[IndexOfMaxY-1];
	double beta = vD[IndexOfMaxY];
	double gamma = vD[IndexOfMaxY+1];
	double delta = 0.5 * ((alpha - gamma) / (alpha - (2.0 * beta) + gamma));
	Serial.println(String(alpha) + " " + String(beta) + " " + String(gamma)+" " + String(delta));
	double interpolatedX = beta - (alpha - gamma) / 4.0 * delta;
	/* retuned value: interpolated peak amplitude */
	return(interpolatedX);
}
